...
# ====================================================================
# 仅在首次创建配置时注入 IM 渠道、工具、网关等设置。
# 后续重启不再覆盖，以保留 Gateway 运行时的修改（如配对审批等）。
# 如需强制重新生成配置，删除 $CONFIG_FILE 后重启容器即可。
#
# 注意：大模型凭证（API Key/Base URL/Model）始终同步，不受此限制。
# ====================================================================

# ==============================================================================
# 2. 大模型提供商(Provider)与模型(Model)同步（每次重启均同步）
# ==============================================================================
...
# 以下 IM 渠道、工具、网关配置仅在首次启动时写入
if [[ "$CONFIG_FRESH" == "true" ]]; then
    log_info "首次启动，从环境变量注入 IM 渠道与网关配置..."
...
rm -f "$tmp_file"
    ...
    log_info "配置文件已存在，跳过环境变量注入（保留运行时修改）。如需重新生成，请删除 $CONFIG_FILE 后重启。"
fi  # end CONFIG_FRESH
